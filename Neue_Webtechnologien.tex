...
\subparagraph{Der Aufruf} dieses Polymer-Elements muss wie im Beispiel \ref{Element Attribute benutzen} umgesetzt werden.
...
Service Worker werden zur Zeit schon den aktuellen Versionen von Chrome, Firefox und Opera unterstützt. Für Safari und Edge, ist die Unterstützung momentan noch in der Entwicklung. \cite{SWSupp1} \cite{SWSupp2}
...
Ist dies der Fall erledigt er Service Worker diese und geht danach wieder in den Leerlauf.
...
Im Codebeispiel \ref{sw_reg} wird überprüft, ob der aktuelle Browser Service Worker und Push-Nachrichten unterstützt. Ist dies der Fall, wird der Service Worker, welcher sich in der Datei \texttt{sw.js} befindet installiert.

\lstinputlisting[language=JavaScript, caption=Service Worker Hauptfunktion, label=sw_main, captionpos=b]{Code/sw.js}

Innerhalb des Service Worker ist die im Codebeispiel \ref{sw_main} gezeigte Funktion dafür verantwortlich, Nachrichten zu empfangen und dem Nutzer anzuzeigen.
...
Der beste Weg Polymer-Elemente in Typo3 zu integrieren ist, diese in \textit{ViewHelper} zu kapseln (siehe Abschnitt \ref{Fluid}). 

Das Ziel ist es also, einen \textit{ViewHelper} zu erstellen, welcher einen \textit{ViewHelper}-Tag in einen Polymer-Tag umwandelt.

\begin{wrapfigure}{r}{4cm}
\centering
\includegraphics[width=4cm]{Bilder/Polymer.eps}
\caption{Polymer \textit{ViewHelper} Hierarchie}
\label{Polymer ViewHelper Hirarchie}
\vspace{-70pt}
\end{wrapfigure}

In Abbildung \ref{Polymer ViewHelper Hirarchie} (rechts) ist die Aufrufhirarchie zu sehen, welche innerhalb des Projektes entwickelt wurde, um Polymer Elemente elegant in Typo3 einzuarbeiten. Die gezeigte Hirarchie läuft innerhalb der Abbildung \ref{Typo3_Call} im Schitt fünf ab.
...
Das Template wiederum beinhaltet einen für das Polymer Element entwickelten \textit{ViewHelper}, welcher innerhalb des Templates aufgerufen wird.

Das Fluid-Framework ruft beim Rendern eines Templates den entsprechenden \textit{ViewHelper} auf. 

Der \textit{ViewHelper} wiederum gibt den Link zum entsprechenden Polymer Element und den Polymer Tag zurück.
...
\newpage
\lstinputlisting[language=php, caption=Polymer \textit{ViewHelper}, label=Polymer ViewHelper, captionpos=b]{Code/viewhelper.php}

Der \textit{ViewHelper} erbt von der abstrakten Klasse "`AbstractTagBasedViewHelper"'. Dieser ist in der \textit{ViewHelper} ist anschließend in der Lage, einen HTML-Tag zu verändern.

Mit dem Attribut \texttt{\$tagName} wird definiert wie der neue Tag heißen soll, welcher den \textit{ViewHelper}-Tag ersetzt. Dieser Tag muss nun so heißen wie das Polymer-Element.

Die Methode \texttt{render} des \textit{ViewHelpers} gibt den String zurück, der den \textit{ViewHelper}-Tag ersetzt.

Die "`return"'-Anweisung gibt immer den Link zum entsprechenden Polymer-Element zurück, und den Polymer-Tag.

Der \textit{ViewHelper} wird nun wie folgt aufgerufen.

\lstinputlisting[language=html, caption=Polymer \textit{ViewHelper} aufrufen, label=Polymer ViewHelper aufrufen, captionpos=b]{Code/viewhelper_call.php}

Beim ausliefern der Web-Seite ersetzt Typo3 nun den \textit{ViewHelper}-Tag gegen die folgenden HTML-Elemente

\lstinputlisting[language=html, caption=Polymer \textit{ViewHelper} Ersetzung, label=Polymer ViewHelper Ersetung, captionpos=b]{Code/viewhelper_after.php}
...
Durch die Verwendung von \textit{ViewHelpern} kann jedes beliebige Polymer-Element auch unter Typo3 zum Einsatz kommen. Es ist also sehr elegant möglich Boiler-Code auch in Typo3-Templates und -Partial mit der Hilfe von Polymer zu umgehen.

Durch die Kapselung der Polymer-Elemente in \textit{ViewHelpern} verhalten sich diese außerdem wie standardmäßige \textit{ViewHelper} mit Boiler-Code.

Um nun Polymer-Elemente in einem Template zu verwenden, ist kein extra Wissen notwendig, was eine Verwendung so einfach wie möglich macht. Der Programmierer eines Templates muss sich keine Gedanken über Links zu Polymer-Elementen machen und muss sich auch nicht mit Polymer auskennen. Es wird einfach der \textit{ViewHelper} verwendet und alles andere passiert im Hintergrund.
...
Leider gibt es keine bestehende Extension im \ac{TER}, die diesen Use Case auch nur im Ansatz umsetzt, weshalb entschieden wurde, die Registration für Nutzer selbst zu entwickeln.
...
Die TYPO3 eigene Extension \textit{FE\_Login} ist nur schwer zu konfigurieren und bietet keine Möglichkeit den Login in modale Dialoge auszulagern. Aus diesen Gründen wurde entschieden, den Login durch einen eigenen Controller innerhalb der \textit{SupportBase}-Extension zu realisieren.
...
Im Quellcodebeispiel \ref{loginAction} wird der User anhand des Namens aus der Datenbank geholt (Zeile 4). 
...
Durch die gewonnenen Erkentnisse konnte nicht nur die Basis für ein Supportportal geschaffen werden. Es wurde Testweise auch ein weiteres Modul, das Download-Portal implementiert, um die genannten Behauptungen zu belegen.
